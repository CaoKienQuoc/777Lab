...
function renderBlogList(posts) {
  ...
  const items = posts.map(p => {
    ...
    const displayDate = `<span>${formatDateTime(p.createdAt)}</span>`
      + (p.scheduledAt ? ` · <span>📅 ${t('schedulePost')}: ${formatDateTime(p.scheduledAt)}</span>` : '');
...
async function openBlogDetail(id) {
  try {
    ...
    const scheduledLine = p.scheduledAt
      ? ` · 📅 ${t('schedulePost')}: ${formatDateTime(p.scheduledAt)}`
      : '';
    const body = `
      <div class="post-detail">
        <h2>${escapeHtml(p.title)}</h2>
        <div class="post-meta">${escapeHtml(p.authorName)} · ${formatDateTime(p.createdAt)}${scheduledLine}</div>
